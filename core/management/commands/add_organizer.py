...
class Command(BaseCommand):
    ...
    def get_organizer_data(self):
        ...
        try:
            data = {'first_name': main_name.split(' ')[0], 'last_name': main_name.split(' ')[1], 'email': main_email}
        except IndexError:
            data = {'first_name': main_name, 'last_name': '', 'email': main_email}
    ...
    def handle(self, *args, **options):
        event_id = click.prompt(click.style("What's the event ID? NOT the event page ID. We want EVENT ID here", bold=True, fg='yellow'))
        event = Event.objects.get(id=event_id)
        click.echo("Ok, we're adding to an event in {}, {}".format(event.city, event.country))

        team = [self.get_organizer_data()]

        while click.confirm(click.style("Do you want to add additional team members?", bold=True, fg='yellow'), default=False):
        ...
        for member in members_as_list:
            if 'password' in member:
                click.echo("{} - email: {} password {}".format(member['first_name'], member['email'], member['password']))
            else:
                click.echo("{} - email: {} already has account".format(member['first_name'], member['email']))                
