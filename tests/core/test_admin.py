...
def test_adding_organizer_as_superuser(
        invite_to_slack, admin_client, future_event, hidden_event, django_user_model):
    ...
    data = {
        ...
        'name': 'New organizer',
        'email': 'new-superuser@organizer.com'
    }
...
@mock.patch('core.models.User.invite_to_slack')
def test_organizer_can_only_add_to_their_event(
        invite_to_slack, client, organizer_peter, future_event, hidden_event):
    add_organizers_url = reverse('admin:core_event_add_organizers')
    client.force_login(organizer_peter)
    data = {
        'event': hidden_event.pk,
...
def test_remove_organizer_as_superuser(
        admin_client, organizer_julia, organizer_peter, future_event):
...
def test_organizers_cannot_remove_themselves(
        client, organizer_peter, future_event, organizer_julia):
    future_event.team.add(organizer_julia)
    manage_organizers_url = reverse('admin:core_event_manage_organizers')
    client.force_login(organizer_peter)
    data = {
