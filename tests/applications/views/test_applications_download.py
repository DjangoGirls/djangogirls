...
def test_download_applications_list(
    ...
    assert (
        ...
        'attachment; filename="{}.csv"'.format(future_event.page_url))
...
def test_download_applications_list_with_question_added(
        admin_client, application_submitted, future_event, future_event_form,
        applications):
    applications_url = reverse(
        'applications:applications_csv',
        kwargs={'city': future_event.page_url})
    ...
    new_application = Application.objects.create(
        form=future_event_form, state='submitted')
    new_application_questionx_answer = Answer.objects.create(
        application=new_application,
        question=questionx,
        answer='answer to questionx for app 5')
    new_application_5_last_answer = Answer.objects.create(
        application=new_application,
