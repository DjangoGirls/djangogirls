        ...
        for month, month_data in sorted(data.items(), key=lambda t: t[0]):
            for reward_t, patrons in month_data.items():
                if options['create_rewards']:
                    defaults = {'description': reward_t.description,
                                'value': reward_t.value}
                    reward, created = Reward.objects.get_or_create(
                        name=reward_t.name, defaults=defaults)
                    if created:
                        logging.info(
                            "Created reward object with name %r.", reward.name)
                else:
                ...
                for patron_t in patrons:
                    ...
                    payment, created = Payment.objects.get_or_create(
                        patron=patron, month=month, defaults=defaults)
                    if created:
